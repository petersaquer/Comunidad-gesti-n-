...
export const UserBadge: React.FC<UserBadgeProps> = ({
  ...
  const getRoleIcon = () => {
    ...
    switch (role) {
      ...
      case 'vicepresidente':
        return <Crown className={`${iconClasses} text-yellow-600 shrink-0`} />;
      case 'tesorera':
        return <Coins className={`${iconClasses} text-emerald-600 shrink-0`} />;
      case 'subtesorera':
        return <Coins className={`${iconClasses} text-teal-600 shrink-0`} />;
      case 'secretaria':
        return <FileText className={`${iconClasses} text-indigo-600 shrink-0`} />;
      case 'subsecretaria':
        return <FileText className={`${iconClasses} text-purple-600 shrink-0`} />;
      case 'delegado':
      ...
      case 'sindico_suplente':
        return <Scale className={`${iconClasses} text-sky-600 shrink-0`} />;
      case 'directiva':
        return <Shield className={`${iconClasses} text-blue-600 shrink-0`} />;
      case 'vocal_suplente':
        return <Shield className={`${iconClasses} text-slate-600 shrink-0`} />;
      case 'residente':
      default:
  ...
  const badgeThemeClasses: Record<UserRole, string> = {
    admin:
      'bg-linear-to-r from-amber-50 via-amber-100/70 to-yellow-50 text-amber-900 border border-amber-300 shadow-2xs',
    vicepresidente:
      'bg-linear-to-r from-yellow-50 via-amber-50 to-yellow-100/70 text-yellow-950 border border-yellow-400 shadow-2xs',
    tesorera:
      'bg-linear-to-r from-emerald-50 via-teal-50 to-emerald-100/60 text-emerald-900 border border-emerald-300 shadow-2xs',
    subtesorera:
      'bg-linear-to-r from-teal-50 via-emerald-50 to-teal-100/60 text-teal-950 border border-teal-300 shadow-2xs',
    secretaria:
      'bg-linear-to-r from-indigo-50 via-purple-50 to-indigo-100/60 text-indigo-900 border border-indigo-300 shadow-2xs',
    subsecretaria:
      'bg-linear-to-r from-purple-50 via-indigo-50 to-purple-100/60 text-purple-950 border border-purple-300 shadow-2xs',
    delegado:
    ...
    sindico_suplente:
      'bg-linear-to-r from-sky-50 via-cyan-50 to-sky-100/60 text-sky-950 border border-sky-300 shadow-2xs',
    directiva:
      'bg-linear-to-r from-blue-50 via-indigo-50 to-blue-100/60 text-blue-950 border border-blue-300 shadow-2xs',
    vocal_suplente:
      'bg-linear-to-r from-slate-100 via-blue-50 to-slate-200/70 text-slate-800 border border-slate-300 shadow-2xs',
